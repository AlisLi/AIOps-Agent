...
sequenceDiagram
    ...
    Note over QA,HL: 所有事件 Key = trace_id / alert_id，落同一分区
```
...
flowchart TD
    ...
    C -- dist < 0.08 --> HIT[返回缓存 RAGResponse<br/>from_cache=true]
    ...
    S2 --> P
    S3 --> P
    P --> VR[Vector 召回<br/>Milvus / 内存 cosine]
    P --> BR[BM25 召回<br/>rank-bm25 + jieba]
    VR --> F[RRF 融合<br/>score = Σ w_i / (k+rank_i)]
    BR --> F
...
stateDiagram-v2
    ...
    Appending --> WithinWindow: len(messages) ≤ N
    Appending --> Evicting: len(messages) > N
    WithinWindow --> Idle
    Evicting --> Summarizing: spawn asyncio.create_task
    Summarizing --> Merged: LLM(old_summary + evicted)
...
flowchart TD
    ...
    W --> A[3-Sigma 判定<br/>|x-mean| > k*std]
    W --> B[EWMA 判定<br/>α=0.3, 偏离 > k*std]
    A -->|true| V1((+1))
    A -->|false| V0a((0))
    B -->|true| V2((+1))
    B -->|false| V0b((0))
    ...
    V2 --> S
    V0b --> S
    S -->|count ≥ 1| AL[构造 AlertEvent<br/>detected_by: vote/3sigma/ewma]
...
flowchart LR
    ...
    P -->|match panic/fatal| E1[severity=P1]
...
sequenceDiagram
    ...
    N-->>RCA: [{kind, name, relation}]
    RCA->>RCA: build_query(alert + subgraph)
    RCA->>RAG: RAGRequest (category=RESOURCE/LOG)
    RAG-->>RCA: Top-5 RetrievedDoc
    RCA->>LLM: render(rca.jinja) → JSON
    alt LLM 可用
        LLM-->>RCA: {root_cause_service, reasoning, suggested_actions}
    else LLM / 解析失败
        RCA-->>RCA: 降级: service=alert.service,<br/>action=restart_pod
    end
    RCA->>OUT: RCAResult
    RCA->>OUT: HealRequest (runbook=actions[0])
```
...
stateDiagram-v2
    [*] --> CLOSED
    CLOSED --> CLOSED: 调用成功<br/>failures=0
    CLOSED --> OPEN: failures ≥ threshold<br/>记录 opened_at
    OPEN --> OPEN: 调用请求<br/>直接抛 CircuitOpenError
    OPEN --> HALF_OPEN: now - opened_at<br/>≥ recovery_timeout
    HALF_OPEN --> CLOSED: 探测调用成功
    HALF_OPEN --> OPEN: 探测调用失败<br/>重置 opened_at
    note right of OPEN
      快速失败阶段
      保护下游不被打爆
    end note
...
flowchart LR
    ...
    H --> CB
    CB -->|成功| OK[返回结果]
    CB -->|CircuitOpenError| FB[走离线/降级路径]
```
...
flowchart TD
    subgraph Online["在线 (in-request)"]
      ...
      K --> ST[(存储: ClickHouse /<br/>ElasticSearch)]
      ST --> DASH[Grafana / BI 看板]
    end
    subgraph Offline["离线 (batch)"]
      ...
      RAGAS --> GATE{CI 门槛<br/>faithfulness≥0.80<br/>context_recall≥0.75}
      LITE --> GATE
      GATE -->|pass| REL[允许发布]
      GATE -->|fail| BLK[阻断发布]
    end
